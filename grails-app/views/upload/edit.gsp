...
<div class="inner">
    ...
    <section class="sightings-block ui-corner-all">
        <a href="http://bie.ala.org.au/species/${taxonConceptID}" target="_blank">
            <img id="taxonImage" class="taxon-image ui-corner-all" src="${imageUrl}"/>
        </a>
        <div class="left ${taxonConceptID?'':'hidden'}" id="taxonBlock" style="width:53%;padding-top:15px;">
        ...
        <div class="left" style="width:35%;">
            <p class="${taxonConceptID?'':'hidden'}" id="changeTaxonText">Not the right species? To change identification, type a scientific or common name into
            the box below and choose from the auto-complete list.</p>
            <p class="${taxonConceptID?'hidden':''}" id="chooseTaxonText">Type a scientific or common name into the box below and choose from the auto-complete list.</p>
            <input type="text" value="" id="taxa" name="taxonText" class="name_autocomplete ac_input" style="width:75%" autocomplete="off">
            <input type="hidden" name="lsid" id="lsid" value="${taxonConceptID}">
            <button class="ui-state-disabled ${taxonConceptID?'':'hidden'}" type="button" id="undoTaxon" disabled="disabled">Undo</button>
        </div>
    </section>
    <!-- WHEN -->
    ...
    <section class="sightings-block ui-corner-all">
        <div class="left" id="location-container">
            <div class="left">
                ...
                <g:textArea class="left" name="location" rows="5" cols="30" value="${locality}"/>
            </div>
        ...
        <div class="left" id="coordinate-container">
            ...
            <g:select name="georeferenceProtocol" from="['Google maps','Google earth','GPS device','camera/phone','physical map','other']"
                      value="${georeferenceProtocol}"/><br/>
            <div id="precisionFields">
                <span id="geodeticDatumField" class="ui-helper-hidden"><label for="geodeticDatum">Enter the geodetic datum for your device</label>
                    <g:select name="geodeticDatum" from="['WGS84','GDA94','AGD 1966','AGD 1984','other','unknown']" value="${geodeticDatum}"/>
    ...
    <!-- existing media -->
    <section id="existingMedia">
        <g:if test="${images}">
            <span id="existingMediaLabel">Existing media</span>
        </g:if>
        <ul style="display: inline;">
            <g:each in="${images}" var="image" status="idx">
                <li id="${associatedMedia[idx][associatedMedia[idx].lastIndexOf('\\')+3..-1]}">
